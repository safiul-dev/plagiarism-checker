...
class PlagiarismChecker:
    ...
    def santenceSimilarity(self, text1, text2):
        ...
        self.sentence_count = len(similer_text)
        return similer_text 
        ...
        for i in range(len(text1)):
            ...
            for j in range(len(text2)):
                ...
                if (par > pre):
                    pre = par
            sum += pre
        first_text_length = len(text1)
        second_text_length = len(text2)
        final_length = first_text_length
        if (self.sentence_count == first_text_length) :
            final_length = first_text_length
        elif (self.sentence_count == second_text_length) :
            final_length = second_text_length
        else :
            final_length = max(first_text_length, second_text_length)
        
        result = "{:.2f}".format(sum / final_length)
        if ( float(result) > 100 ) :
            return 100.00
        return result
